const MoviesData = {
  ...
  const searchBtn = document.getElementById('searchByBtn');
  ...
  function displayMovies(page) {
    ...
    page.results.forEach((element) => {
      ...
      imageTitle.style.width = '150px';
      const movieTitle = document.createElement('p');
      const votes = document.createElement('p');
      const rating = document.createElement('p');
      const heartLiked = document.createElement('i');
  
      votes.textContent = `Votes: ${element.vote_count}`;
      rating.textContent = `Rating: ${element.vote_average}`;
      votes.style.textAlign = 'left';
      votes.style.marginLeft = '5px';
      rating.style.marginLeft = '5px';
      rating.style.textAlign = 'left';
      votes.style.fontSize = '14px';
      rating.style.fontSize = '14px';
      imageTitle.src = element.poster_path;
      movieTitle.textContent = element.original_title;
      movieTitle.style.fontSize = '14px';
      movieTitle.style.fontWeight = 'bold';
  

      if (Favourites.some((favMovie) => favMovie.id === element.id)) {
        heartLiked.classList.add('fa-solid', 'fa-heart');
      } else {
        heartLiked.classList.add('fa-regular', 'fa-heart');
      }
  

      heartLiked.addEventListener('click', () => {
        const isFavorite = Favourites.some((favMovie) => favMovie.id === element.id);
        if (!isFavorite) {
          Favourites.push(element);
          heartLiked.classList.remove('fa-regular');
          heartLiked.classList.add('fa-solid');
        } else {
          const index = Favourites.findIndex((favMovie) => favMovie.id === element.id);
          if (index !== -1) {
            Favourites.splice(index, 1);
          }
          heartLiked.classList.remove('fa-solid');
          heartLiked.classList.add('fa-regular');
        }
      });
  
      movieElement.append(imageTitle, movieTitle, votes, rating, heartLiked);
      movieElement.style.border = '1px solid black';
      listMovies.append(movieElement);
    });
  }
  
  function displayPage(pageNumber) {

    if (pageNumber < 1 || pageNumber > MoviesData.page1.total_pages) {
    ...
    document.getElementById('current').textContent = `Current page: ${currentPage}`;
  

    const pageData = MoviesData[`page${currentPage}`];
  ...
  function sortByReleaseYear() {
    const sortedMovies = MoviesData[`page${currentPage}`].results.sort((a, b) => {
      const dateA = new Date(a.release_date);
      const dateB = new Date(b.release_date);
      return dateA - dateB;
    });
  
    displayMovies({ ...MoviesData[`page${currentPage}`], results: sortedMovies });
  }
    function sortByRating() {
    const sortedMovies = MoviesData[`page${currentPage}`].results.sort((a, b) => a.vote_average - b.vote_average);
    displayMovies({ ...MoviesData[`page${currentPage}`], results: sortedMovies });
  }
  
  function searchMovies(searchTerm) {
    const filteredMovies = MoviesData[`page${currentPage}`].results.filter(
      (movie) => movie.original_title.toLowerCase().includes(searchTerm.toLowerCase())
    );
    displayMovies({ ...MoviesData[`page${currentPage}`], results: filteredMovies });
  }
  ...
  function showFavorites() {
    displayMovies({ page: 1, results: Favourites });
  }
  

  function nextPage() {
    if (currentPage < MoviesData.page1.total_pages) {
      currentPage++;
      displayPage(currentPage);
    }
  }
  ...
  sortDateBtn.addEventListener('click', sortByReleaseYear);
  ...
  allTab.addEventListener('click', showAllMovies);
  favoritesTab.addEventListener('click', showFavorites);
